...
void analisi_recursiva(char* path, char* file_name){
	...
	while ((fichero = readdir(dirp)) != NULL){
		...
		//comprobamos si puedo acceder al dicho archivo en el directorio y a su atributos (almacenados en &atributos)
	        if(stat(fichero->d_name,&atributos) < 0) {
	                printf("Error al intentar acceder a los atributos de %s%s\n", temp_path, fichero->d_name);

	        } else {
			//Si puedo acceder a el archivo, compruebo si es un fichero regulare o un directorio

	                if(fichero->d_type!=DT_DIR){
				//Si es un fichero regulare
				...
				//Si un nuevo directorio, llama se misma (recursividad)
				if (strcmp(fichero->d_name,".")!=0 && strcmp(fichero->d_name,"..")!=0){
					analisi_recursiva(temp_path,strcat(fichero->d_name,"/"));
					//Cuando sale de la recursiva, tiene que salir de un directorio tambien
					chdir("../");
				}
			}
	        }
